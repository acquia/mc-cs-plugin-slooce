...
def SUBMODULE_NAME = "MauticSlooceTransportBundle"
    ...
    stage('Download and combine') {
      steps {
        container('hosted-tester') {
          ...
          sh('rm -r plugins/MauticSlooceTransportBundle || true; mkdir -p plugins/MauticSlooceTransportBundle && chmod 777 plugins/MauticSlooceTransportBundle')
          dir('plugins/MauticSlooceTransportBundle') {
            checkout scm
          }
        }
      }
        ...
        container('hosted-tester') {
          ansiColor('xterm') {
            sh """
              ...
              bin/phpunit -d memory_limit=2048M --bootstrap vendor/autoload.php --fail-on-warning  --testsuite=all --configuration plugins/MauticSlooceTransportBundle/phpunit.xml
            """
          }
        }
      }
